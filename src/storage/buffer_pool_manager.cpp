    ...
    if (replacer_->victim(frame_id)) {
        ...
        if (replace_page_no != -1) {
            ...
            if (replace_page->is_dirty_) {
                disk_manager_->write_page(replace_page->id_.fd, replace_page_no, replace_page->data_, PAGE_SIZE);
...
Page* BufferPoolManager::fetch_page(PageId page_id) {
    ...
    if(replace_page->is_dirty_) {
        disk_manager_->write_page(replace_page->id_.fd, replace_page->id_.page_no, replace_page->data_, PAGE_SIZE);
